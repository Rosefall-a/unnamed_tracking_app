...
async function requestReset() {
  ...
  try {
    ...
    if (!response.ok)
      throw new Error(getApiError(data, "Unable to request a reset."));
    message.value =
      (data as { message?: string }).message ??
      "If an account matches that information, a password reset email has been sent.";
  } catch (e) {
...
async function resetPassword() {
  ...
  try {
    ...
    const data: unknown = await response.json();
    // Never display a response body for a request containing a new password.
    ...
    if (!currentUser.value || currentUser.value.id !== userId) {
      throw new Error(
        "Password reset completed, but the new sign-in session could not be verified.",
      );
    }
...
<template>
  <main class="page">
    <form
      class="card"
      @submit.prevent="mode === 'request' ? requestReset() : resetPassword()"
    >
      <div class="brand">
        <span>🎮</span>
        <h1>Archive</h1>
      ...
      <template v-if="mode === 'request'">
        ...
        /></label>
        <p class="hint">
          If an account matches, we'll send a password reset link to its email
          address.
        </p>
      </template>
      <template v-else>
