...
async function requestReset() {
  ...
  if (!identifier.value.trim()) {
    error.value = "Enter your username or email.";
    return;
  }
  loading.value = true;
  try {
  ...
  <main class="page">
    <form class="card" @submit.prevent="mode === 'request' ? requestReset() : resetPassword()">
      ...
      <template v-if="mode === 'request'">
        <label><span>Username or email</span><input v-model="identifier" autocomplete="username" required /></label>
        <p class="hint">If an account matches, we'll send a password reset link to its email address.</p>
      </template>
      <template v-else>
        <label><span>New password</span><input v-model="password" type="password" autocomplete="new-password" minlength="9" required /></label>
      ...
      <button :disabled="loading">{{ loading ? (mode === "request" ? "Sending…" : "Resetting…") : (mode === "request" ? "Send reset link" : "Reset password") }}</button>
