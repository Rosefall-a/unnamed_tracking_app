...
async function sso() { oidcLoading.value = true; error.value = null; try { startOidcLogin(); } catch (err) { error.value = err instanceof Error ? err.message : "Unable to start SSO."; oidcLoading.value = false; } }
</script>
<template><main class="login-page"><form class="login-card" @submit.prevent="submit">
  ...
  <label v-if="oidcAvailable" class="method-field"><span>Sign-in method</span><select v-model="loginMethod"><option value="sso">SSO</option><option value="local">Local username & password</option></select></label>

  <template v-if="loginMethod === 'sso' && oidcAvailable">
    <p class="sso-description">Continue using your configured identity provider.</p>
    <div v-if="error" class="login-error">{{ error }}</div>
    <button type="button" class="oidc-button primary" :disabled="oidcLoading" @click="sso">{{ oidcLoading ? "Opening SSO…" : ssoButtonText }}</button>
  </template>
  <template v-else>
...
<style scoped>
.login-page{min-height:100vh;display:flex;align-items:center;justify-content:center;background:#121212;font-family:system-ui,sans-serif;position:relative;overflow:hidden}.login-page::before{content:"";position:absolute;width:600px;height:600px;background:radial-gradient(circle,rgba(214,138,52,.18) 0%,transparent 70%);top:50%;left:50%;transform:translate(-50%,-50%)}.login-card{position:relative;z-index:1;width:100%;max-width:360px;background:rgba(26,26,26,.9);backdrop-filter:blur(12px);border:1px solid #2a2a2a;border-radius:14px;padding:32px;box-shadow:0 24px 64px rgba(0,0,0,.5);display:flex;flex-direction:column;gap:16px}.login-brand{display:flex;align-items:center;gap:10px;justify-content:center}.brand-icon{font-size:26px}.login-brand h1{margin:0;color:#fff;font-size:1.4rem}.login-subtitle{margin:-8px 0 4px;color:#999;font-size:13px;text-align:center}.method-field,.field{display:flex;flex-direction:column;gap:6px;font-size:.85rem;color:#ccc}.method-field select,.field input{background:#111;border:1px solid #3a3a3a;border-radius:8px;color:#fff;padding:10px 12px;font:inherit}.method-field select:focus,.field input:focus{outline:none;border-color:#d68a34}.sso-description{margin:0;color:#999;font-size:13px;text-align:center}.login-error{color:#fca5a5;font-size:13px;background:rgba(220,38,38,.1);border:1px solid rgba(220,38,38,.3);border-radius:8px;padding:8px 10px}.login-button,.oidc-button{border:0;border-radius:8px;padding:11px;font-weight:600;cursor:pointer}.login-button{background:#d68a34;color:#111}.login-button:disabled,.oidc-button:disabled{opacity:.6;cursor:not-allowed}.oidc-button{background:#2a2a2a;color:#fff;border:1px solid #444}.oidc-button.primary{background:#d68a34;color:#111;border-color:#d68a34}
</style>
