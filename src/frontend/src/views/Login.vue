...
onMounted(async () => {
  const resetStatus = await fetch("/api/auth/password-reset/status")
    .then(async (response) =>
      response.ok ? response.json() as Promise<{ enabled: boolean }> : { enabled: false },
    )
    .catch(() => ({ enabled: false }));
  ...
  <main class="login-page">
    <form class="login-card" @submit.prevent="submit">
      ...
      <template v-if="localOnly">
        ...
        <button type="submit" class="login-button" :disabled="loading">{{ loading ? "Signing in…" : "Sign in" }}</button>
      ...
      <template v-else-if="loginMethod === 'local' || !oidcAvailable">
        ...
        <div v-if="oidcAvailable" class="provider-buttons">
          <button v-for="provider in oidcProviders" :key="provider.slug" type="button" class="oidc-button" :style="buttonStyle(provider)" :disabled="oidcLoading" @click="sso(provider.slug)">
            <img v-if="provider.button_image_url" :src="provider.button_image_url" alt="" /><span>{{ provider.button_text || provider.name }}</span>
          </button>
        </div>
      ...
      <template v-else>
        ...
        <div class="provider-buttons">
          <button v-for="provider in oidcProviders" :key="provider.slug" type="button" class="oidc-button primary" :style="buttonStyle(provider)" :disabled="oidcLoading" @click="sso(provider.slug)">
            <img v-if="provider.button_image_url" :src="provider.button_image_url" alt="" /><span>{{ provider.button_text || provider.name }}</span>
          </button>
        </div>
        ...
        <details class="local-credentials"><summary>Use local credentials</summary><div class="local-fields"><label class="field"><span>Username or email</span><input v-model="usernameOrEmail" type="text" autocomplete="username" /></label><label class="field"><span>Password</span><input v-model="password" type="password" autocomplete="current-password" /></label><button type="submit" class="login-button" :disabled="loading || oidcLoading">{{ loading ? "Signing in…" : "Sign in locally" }}</button><router-link v-if="passwordResetAvailable" class="forgot-link" to="/reset-password">Forgot your password?</router-link></div></details>
      </template>
    </form>
  </main>
</template>
