  ...
  <main class="login-page">
    <form class="login-card" @submit.prevent="submit">
      ...
      <template v-if="loginMethod === 'local' || !oidcAvailable">
        ...
        <button v-if="oidcAvailable && !oidcProviders.length" type="button" class="oidc-button" :disabled="oidcLoading" @click="() => sso()"><span>{{ oidcLoading ? "Opening SSO…" : ssoButtonText }}</span></button>
      </template>
      <template v-else>
        ...
        <details class="local-credentials">
          ...
          <div class="local-fields">
            <label class="field"><span>Username or email</span><input v-model="usernameOrEmail" type="text" autocomplete="username" /></label>
            <label class="field"><span>Password</span><input v-model="password" type="password" autocomplete="current-password" /></label>
            <button type="submit" class="login-button" :disabled="loading || oidcLoading">{{ loading ? "Signing in…" : "Sign in locally" }}</button>
          </div>
        </details>
      </template>
    </form>
