...
const oidcMessages: Record<string, string> = {
  ...
  verified_email_required: "Your SSO account must provide a verified email address.",
  account_disabled: "This account is disabled.",
  identity_missing: "Your SSO account did not provide the identity field required for account matching.",
  identity_conflict: "This SSO identity is already linked to another account.",
  user_creation_disabled: "Your SSO account is not registered and automatic account creation is disabled.",
};
...
onMounted(async () => {
  ...
  loginMethod.value = oidc.enabled && oidc.default_login_method === "sso" ? "sso" : "local";
    ...
    <form class="login-card" @submit.prevent="submit">
      ...
      <template v-if="loginMethod === 'local' || !oidcAvailable">
        ...
        <div v-if="oidcAvailable" class="provider-buttons">
          <button v-for="provider in oidcProviders" :key="provider.slug" type="button" class="oidc-button" :disabled="oidcLoading" @click="() => sso(provider.slug)"><img v-if="provider.button_image_url" :src="provider.button_image_url" alt="" /><span>{{ provider.button_text || provider.name }}</span></button>
        </div>
      </template>
      <template v-else>
        ...
        <div class="provider-buttons">
          <button v-for="provider in oidcProviders" :key="provider.slug" type="button" class="oidc-button primary" :disabled="oidcLoading" @click="() => sso(provider.slug)"><img v-if="provider.button_image_url" :src="provider.button_image_url" alt="" /><span>{{ provider.button_text || provider.name }}</span></button>
        </div>
        ...
        <div class="local-switch"><label for="login-method">Use local credentials</label><select id="login-method" v-model="loginMethod"><option value="sso">SSO</option><option value="local">Local username &amp; password</option></select></div>
      </template>
    </form>
  </main>
</template>
