...
function sso(slug?: string) {
  oidcLoading.value = true; error.value = null;
  try { startOidcLogin(slug); } catch (err) { error.value = err instanceof Error ? err.message : "Unable to start SSO."; oidcLoading.value = false; }
}

function buttonStyle(provider: OidcLoginProvider) {
  const hex = (provider.button_color || "#d68a34").slice(1); const r = parseInt(hex.slice(0, 2), 16); const g = parseInt(hex.slice(2, 4), 16); const b = parseInt(hex.slice(4, 6), 16);
  return { backgroundColor: provider.button_color || "#d68a34", borderColor: provider.button_color || "#d68a34", color: (0.299 * r + 0.587 * g + 0.114 * b) > 150 ? "#111" : "#fff" };
}
</script>
...
<template>
  <main class="login-page">
    <form class="login-card" @submit.prevent="submit">
      <div class="login-brand"><span class="brand-icon">🎮</span><h1>Archive</h1></div>
      ...
      <template v-if="localOnly">
        ...
        <label class="field"><span>Password</span><input v-model="password" :type="showPassword ? "text" : "password"" autocomplete="current-password" required /></label>
        <label class="password-toggle"><input v-model="showPassword" type="checkbox" /> Show password</label>
        <div v-if="error" class="login-error">{{ error }}</div><button type="submit" class="login-button" :disabled="loading">{{ loading ? "Signing in…" : "Sign in" }}</button>
        <router-link v-if="passwordResetAvailable" class="forgot-link" to="/reset-password">Forgot your password?</router-link>
      </template>
      <template v-else-if="loginMethod === 'local' || !oidcAvailable">
        <label class="field"><span>Username or email</span><input v-model="usernameOrEmail" type="text" autocomplete="username" required /></label>
        <label class="field"><span>Password</span><input v-model="password" :type="showPassword ? "text" : "password"" autocomplete="current-password" required /></label>
        <label class="password-toggle"><input v-model="showPassword" type="checkbox" /> Show password</label>
        <div v-if="error" class="login-error">{{ error }}</div><button type="submit" class="login-button" :disabled="loading || oidcLoading">{{ loading ? "Signing in…" : "Sign in" }}</button>
        <router-link v-if="passwordResetAvailable" class="forgot-link" to="/reset-password">Forgot your password?</router-link>
        <div v-if="oidcAvailable" class="sso-divider"><span>or</span></div><div v-if="oidcAvailable" class="provider-buttons"><button v-for="provider in oidcProviders" :key="provider.slug" type="button" class="oidc-button" :style="buttonStyle(provider)" :disabled="oidcLoading" @click="sso(provider.slug)"><img v-if="provider.button_image_url" :src="provider.button_image_url" alt="" /><span>{{ provider.button_text || provider.name }}</span></button></div>
        <button v-if="oidcAvailable && !oidcProviders.length" type="button" class="oidc-button" :disabled="oidcLoading" @click="sso()">{{ oidcLoading ? "Opening SSO…" : ssoButtonText }}</button>
      </template>
      <template v-else>
        <div class="sso-heading"><span class="sso-icon">◉</span><div><strong>Single sign-on</strong><p>Select an identity provider to continue.</p></div></div>
        <div v-if="error" class="login-error">{{ error }}</div><div class="provider-buttons"><button v-for="provider in oidcProviders" :key="provider.slug" type="button" class="oidc-button primary" :style="buttonStyle(provider)" :disabled="oidcLoading" @click="sso(provider.slug)"><img v-if="provider.button_image_url" :src="provider.button_image_url" alt="" /><span>{{ provider.button_text || provider.name }}</span></button></div>
        <button v-if="!oidcProviders.length" type="button" class="oidc-button primary" :disabled="oidcLoading" @click="sso()">{{ oidcLoading ? "Opening SSO…" : ssoButtonText }}</button>
        <details class="local-credentials"><summary>Use local credentials</summary><div class="local-fields"><label class="field"><span>Username or email</span><input v-model="usernameOrEmail" type="text" autocomplete="username" /></label><label class="field"><span>Password</span><input v-model="password" :type="showPassword ? "text" : "password"" autocomplete="current-password" /></label><label class="password-toggle"><input v-model="showPassword" type="checkbox" /> Show password</label><button type="submit" class="login-button" :disabled="loading || oidcLoading">{{ loading ? "Signing in…" : "Sign in locally" }}</button><router-link v-if="passwordResetAvailable" class="forgot-link" to="/reset-password">Forgot your password?</router-link></div></details>
      </template>
    </form>
  </main>
</template>
