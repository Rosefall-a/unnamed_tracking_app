...
async function loadInvitation() {
  ...
  try {
    ...
    if (!response.ok)
      throw new Error(
        apiError(data, "This invitation is invalid or has expired."),
      );
...
async function acceptInvitation() {
  ...
  try {
    ...
    const data: unknown = await response.json();
    // A new password was submitted, so never render a response body here.
    if (!response.ok) throw new Error("Unable to accept invitation.");
    await checkAuth();
    const userId = (data as { user_id?: string }).user_id;
    if (!currentUser.value || currentUser.value.id !== userId) {
      throw new Error(
        "Your account was created, but the sign-in session could not be verified.",
      );
    }
