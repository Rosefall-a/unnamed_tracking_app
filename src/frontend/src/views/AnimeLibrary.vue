...
const aniListImportResult = ref<{ fetched: number; created: number; updated: number; skipped: number; errors: string[] } | null>(null);
...
async function importFromAniList() {
  ...
  try {
    ...
    aniListImportError.value = e instanceof Error ? e.message : "AniList import failed.";
  } finally {
...
.import-errors { max-height: 120px; overflow: auto; color: #e57373; margin: 0; }
.import-actions { display: flex; justify-content: flex-end; gap: 8px; }
.import-actions button { padding: 8px 14px; border-radius: 7px; cursor: pointer; }
</style>
