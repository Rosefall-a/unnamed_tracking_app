...
function createCollection() {
  createError.value = null
  const name = window.prompt('Name your new collection (use "Parent/Child" to nest it under another):')
...
const STARTER_TEMPLATES = ['Currently Playing', 'Backlog Priority', 'Co-op Games']
...
const collectionSummaries = computed<CollectionSummary[]>(() => {
  ...
  const manual = Array.from(map.entries()).map(([name, list]) => {
    ...
    const pickedIndex = pickedId ? list.findIndex((g) => g.id === pickedId) : -1
    const orderedGames =
      pickedIndex > 0 ? [list[pickedIndex], ...list.slice(0, pickedIndex), ...list.slice(pickedIndex + 1)] : list
...
const STATUS_OPTIONS: GameStatus[] = [
  'wishlist', 'backlog', 'playing', 'on hold', 'beaten', 'played', 'dropped', 'mastered',
]
...
function createSmartCollection() {
  ...
  if (collectionSummaries.value.some((c) => c.name.toLowerCase() === name.toLowerCase())) {
  ...
  addSmartCollection({ name, field: smartField.value, value: smartValue.value.trim() })
  showSmartForm.value = false
}

function deleteSmartCollection(id: string) {
  if (window.confirm('Delete this smart collection? This only removes the rule, no games are affected.')) {
...
<template>
  <main class="collections-page">
      ...
      <div class="header-row">
        ...
        <div class="header-actions">
          ...
          <div class="templates-wrap">
            <button type="button" class="secondary-button" @click="showTemplates = !showTemplates">
              Starter templates
            </button>
            <div v-if="showTemplates" class="templates-dropdown">
              <button
                ...
                type="button"
                class="template-item"
                :disabled="collectionSummaries.some((c) => c.name.toLowerCase() === t.toLowerCase())"
                @click="useTemplate(t)"
              >
                {{ t }}
              </button>
          ...
          <button type="button" class="secondary-button" @click="openSmartForm">+ Smart Collection</button>
    ...
    <div v-if="showSmartForm" class="confirm-backdrop" @click.self="showSmartForm = false">
      <div class="confirm-dialog">
        <h3>New smart collection</h3>
        <p class="dialog-hint">Membership updates automatically as your library changes.</p>
        <label class="field-label">
          Name
          <input v-model="smartName" type="text" class="dialog-input" placeholder="e.g. Mastered Games" />
        </label>
        <label class="field-label">
          Rule
          <select v-model="smartField" class="dialog-input">
            <option v-for="(label, key) in SMART_FIELD_LABELS" :key="key" :value="key">{{ label }}</option>
          </select>
        </label>
        ...
        <label v-else-if="smartField === 'playtime_hours'" class="field-label">
          Hours
          <input v-model="smartValue" type="number" min="0" class="dialog-input" placeholder="e.g. 20" />
        </label>
        <label v-else-if="smartField === 'tag'" class="field-label">
          Tag
          <input v-model="smartValue" type="text" list="smart-tag-options" class="dialog-input" placeholder="e.g. RPG" />
        ...
        <label v-else-if="smartField === 'source'" class="field-label">
          Source
          <input v-model="smartValue" type="text" list="smart-source-options" class="dialog-input" placeholder="e.g. Steam" />
        ...
        <div class="confirm-actions">
          <button type="button" class="secondary-button" @click="showSmartForm = false">Cancel</button>
          <button type="button" class="add-button" @click="createSmartCollection">Create</button>
        </div>
      </div>
    </div>
  </main>
