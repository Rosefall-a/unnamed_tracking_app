import type { Game } from "../types/game";
import eldenRingCover from "../assets/covers/elden-ring.jpg";
import hadesCover from "../assets/covers/hades.jpg";
import hollowKnightCover from "../assets/covers/hollow-knight.jpg";
import eldenRingBanner from "../assets/banners/elden-ring.jpg";
import hadesBanner from "../assets/banners/hades.jpg";
import hollowKnightBanner from "../assets/banners/hollow-knight.jpg";

export const mockGames: Game[] = [
  {
    id: "1",
    title: "Elden Ring",
    coverColor: "#5b3a29",
    coverImageUrl: eldenRingCover,
    bannerImageUrl: eldenRingBanner,
    status: "playing",
    ratingOverall: 9.4,
    ratingStory: 9.2,
    ratingGameplay: 9.8,
    ratingSound: 8.7,
    achievementPercent: 60,
    achievementTotal: 5,
    achievements: [
      { id: "a1", name: "Defeat Margit", unlockedAt: "2026-06-02T19:14:03" },
      { id: "a2", name: "Reach Caelid", unlockedAt: "2026-06-10T21:47:22" },
      { id: "a3", name: "Defeat Godrick", unlockedAt: "2026-07-15T22:03:11" },
      { id: "a4", name: "Defeat Malenia", unlockedAt: null },
      { id: "a5", name: "100% Completion", unlockedAt: null },
    ],
    description:
      "A vast open-world dark fantasy RPG from FromSoftware and George R. R. Martin.",
    developer: "FromSoftware",
    publisher: "Bandai Namco",
    series: null,
    dateAdded: "2026-01-10T12:00:00",
    resumeNote: null,
    lastPlayedAt: "2026-07-15T22:03:11",
    staleSince: null,
    profilesEnabled: false,
    osrsStatsEnabled: false,
    completionDate: null,
    parentGameId: null,
    relationshipType: null,
    folderLocation: "elden-ring",
    releaseDate: "2022-02-25",
    source: "Steam",
    ageRating: "ESRB M",
    timeToBeatHours: 58,
    region: "NA",
    language: "English",
    achievementsProvider: "native",
    links: [],
    ownership: {
      format: "digital",
      purchaseDate: null,
      price: null,
      priceCurrency: null,
      condition: null,
    },
    tags: ["Action RPG", "Souls-like", "Open World"],
    favorite: false,
    collections: ["GOTY Contenders"],
    features: ["Achievements", "Controller Support", "Cloud Saves"],
    platforms: [
      {
        platform: "Steam",
        playtimeMinutes: 8580,
        completionPercent: 87,
        lastPlayedAt: "2026-08-28T20:15:47",
      },
      {
        platform: "PS5",
        playtimeMinutes: 720,
        completionPercent: 4,
        lastPlayedAt: "2026-05-10T14:22:00",
      },
    ],
  },
  {
    id: "2",
    title: "Hades",
    coverColor: "#7a1f2b",
    coverImageUrl: hadesCover,
    bannerImageUrl: hadesBanner,
    status: "mastered",
    ratingOverall: 9.1,
    ratingStory: 8.9,
    ratingGameplay: 9.6,
    ratingSound: 9.3,
    achievementPercent: 100,
    achievementTotal: 4,
    achievements: [
      {
        id: "b1",
        name: "Escape the Underworld",
        unlockedAt: "2026-03-01T18:22:10",
      },
      {
        id: "b2",
        name: "Max Bond with Megaera",
        unlockedAt: "2026-03-20T21:05:33",
      },
      { id: "b3", name: "Clear Heat 32", unlockedAt: "2026-04-05T23:41:02" },
      {
        id: "b4",
        name: "Complete the Codex",
        unlockedAt: "2026-04-18T19:58:47",
      },
    ],
    description: "A rogue-like dungeon crawler from Supergiant Games.",
    developer: "Supergiant Games",
    publisher: "Supergiant Games",
    series: null,
    dateAdded: "2026-02-20T12:00:00",
    resumeNote: null,
    lastPlayedAt: "2026-04-18T19:58:47",
    staleSince: null,
    profilesEnabled: false,
    osrsStatsEnabled: false,
    completionDate: "2026-04-18",
    parentGameId: null,
    relationshipType: null,
    folderLocation: "hades",
    releaseDate: "2020-09-17",
    source: "Xbox Game Pass",
    ageRating: "ESRB T",
    timeToBeatHours: 21,
    region: "NA",
    language: "English",
    achievementsProvider: "native",
    links: [],
    ownership: {
      format: "digital",
      purchaseDate: null,
      price: null,
      priceCurrency: null,
      condition: null,
    },
    tags: ["Roguelike", "Action", "Indie"],
    favorite: false,
    collections: ["GOTY Contenders"],
    features: ["Achievements", "Controller Support", "Cloud Saves"],
    platforms: [
      {
        platform: "Xbox",
        playtimeMinutes: 5640,
        completionPercent: 100,
        lastPlayedAt: "2026-04-18T19:58:47",
      },
    ],
  },
  {
    id: "3",
    title: "Hollow Knight",
    coverColor: "#22303c",
    coverImageUrl: hollowKnightCover, // Hollow Knight
    bannerImageUrl: hollowKnightBanner,
    status: "backlog",
    ratingOverall: null,
    ratingStory: null,
    ratingGameplay: null,
    ratingSound: null,
    achievementPercent: 0,
    achievementTotal: 0,
    achievements: [
      { id: "c1", name: "Reach Dirtmouth", unlockedAt: null },
      { id: "c2", name: "Defeat False Knight", unlockedAt: null },
      { id: "c3", name: "Find the Mothwing Cloak", unlockedAt: null },
    ],
    description:
      "A challenging 2D action-adventure through a vast ruined kingdom.",
    developer: "Team Cherry",
    publisher: "Team Cherry",
    series: null,
    dateAdded: "2026-08-20T12:00:00",
    resumeNote: null,
    lastPlayedAt: null,
    staleSince: null,
    profilesEnabled: false,
    osrsStatsEnabled: false,
    completionDate: null,
    parentGameId: null,
    relationshipType: null,
    folderLocation: "hollow-knight",
    releaseDate: "2017-02-24",
    source: "PlayStation Store",
    ageRating: "ESRB E10+",
    timeToBeatHours: 27,
    region: "PAL",
    language: "English",
    achievementsProvider: "retroachievements",
    links: [],
    ownership: {
      format: "digital",
      purchaseDate: null,
      price: null,
      priceCurrency: null,
      condition: null,
    },
    tags: ["Metroidvania", "Action", "Indie"],
    favorite: false,
    collections: ["Metroidvanias"],
    features: ["Achievements", "Controller Support"],
    platforms: [
      {
        platform: "PlayStation",
        playtimeMinutes: 0,
        completionPercent: null,
        lastPlayedAt: null,
      },
    ],
  },
];
