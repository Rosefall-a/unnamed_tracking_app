...
export interface SmtpSettings {
  enabled:boolean; host:string|null; port:number; username:string|null; password_configured:boolean; use_tls:boolean; use_ssl:boolean; from_email:string|null; from_name:string|null; password_reset_enabled:boolean;
}
...
export async function updateDeploymentSettings(payload:Record<string,string|number|boolean>):Promise<DeploymentSettings>{const response=await fetch("/api/settings/deployment",{method:"PUT",headers:{"Content-Type":"application/json"},credentials:"include",body:JSON.stringify(payload)});if(!response.ok){const message=await response.text();throw new Error(`Failed to save server integrations: ${response.status} ${message}`);}return await response.json();}
