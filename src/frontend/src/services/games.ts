...
export async function fetchGames(): Promise<Game[]> {
  ...
  let skip = 0
  while (true) {
    const response = await fetch(`/api/game/list?skip=${skip}&limit=${GAMES_PAGE_SIZE}`, { credentials: 'include' })
    if (!response.ok) {
...
export async function fetchGameAchievements(id: string): Promise<Achievement[]> {
  ...
  const response = await fetch(`/api/game/${id}/achievements`, { credentials: 'include' })
  if (!response.ok) {
...
export async function fetchAchievementsSummary(): Promise<Record<string, AchievementsSummaryEntry>> {
  ...
  const response = await fetch('/api/game/achievements-summary', { credentials: 'include' })
  if (!response.ok) {
    throw new Error(`Failed to fetch achievements summary: ${response.status} ${response.statusText}`)
  }
...
export async function fetchGame(id: string): Promise<Game | null> {
  ...
  const response = await fetch(`/api/game/get/${id}`, { credentials: 'include' })
...
export async function fetchGameVariants(id: string): Promise<Game[]> {
  ...
  const response = await fetch(`/api/game/${id}/variants`, { credentials: 'include' })
  if (!response.ok) {
    throw new Error(`Failed to fetch variants for game ${id}: ${response.status} ${response.statusText}`)
  }
  const raw: BackendGame[] = await response.json()
...
function mergeField<T>(existing: T | null | undefined, fresh: T | null | undefined, overwrite: boolean): T | null {
...
async function gameAssetExists(gameId: string, assetKind: 'key_art' | 'banner'): Promise<boolean> {
  const response = await fetch(`/api/game/${gameId}/assets/${assetKind}`, { credentials: 'include' })
  return response.ok
}
...
export async function refreshGameMetadata(
  ...
  try {
    ...
    const match = results.find((r) => normalizeTitleForMatch(r.title) === normalizeTitleForMatch(game.title))
    if (!match) {
    ...
    if (options.updateText) {
      ...
      const mergeArr = (existing: string[] | undefined, fresh: string[] | undefined): string[] => {
      ...
      const input: NewGameInput = {
        ...
        timeToBeatHours: mergeField(game.timeToBeatHours, toNumberOrNull(match.time_to_beat_hours), overwrite),
        region: game.region,
...
export async function previewGameMetadataRefresh(
  ...
  const preview: RefreshMetadataPreview = { status: 'error', changedFields: [], wouldAddKeyArt: false, wouldAddBanner: false }
  try {
    const { results } = await searchGameMetadata(game.title)
    const match = results.find((r) => normalizeTitleForMatch(r.title) === normalizeTitleForMatch(game.title))
    if (!match) {
    ...
    if (options.updateText) {
      const textChecks: [string, string | number | null | undefined, string | number | null | undefined][] = [
        ...
        ['time to beat', game.timeToBeatHours, toNumberOrNull(match.time_to_beat_hours)],
      ]
      for (const [label, existing, fresh] of textChecks) {
        if (fresh != null && fresh !== (existing ?? null)) preview.changedFields.push(label)
      }
      if (match.tags?.length && JSON.stringify(match.tags) !== JSON.stringify(game.tags)) preview.changedFields.push('tags')
      if (match.features?.length && JSON.stringify(match.features) !== JSON.stringify(game.features)) preview.changedFields.push('features')
    }
...
export async function listGameNotes(gameId: string): Promise<string[]> {
  ...
  const response = await fetch(`/api/game/${gameId}/notes`, { credentials: 'include' })
  if (!response.ok) {
...
export async function fetchGameNote(gameId: string, noteName: string): Promise<string> {
  ...
  const response = await fetch(`/api/game/${gameId}/notes/${noteName}`, { credentials: 'include' })
  if (!response.ok) {
...
export async function bulkUpdateGames(gameIds: string[], fields: BulkEditFields): Promise<number> {
  const payload: Record<string, unknown> = { game_ids: gameIds }
  if (fields.status !== undefined) payload.status = denormalizeStatus(fields.status)
  ...
  if (!response.ok) {
    const message = await response.text()
    throw new Error(`Failed to bulk-update games: ${response.status} ${response.statusText} ${message}`)
  }
...
async function patchCollections(gameId: string, collections: string[]): Promise<Game> {
  ...
  if (!response.ok) {
    const message = await response.text()
    throw new Error(`Failed to update collections for ${gameId}: ${response.status} ${response.statusText} ${message}`)
  }
  const raw: BackendGame = await response.json()
  return mapBackendGame(raw)
}
...
export async function deleteGame(gameId: string): Promise<void> {
  ...
  const response = await fetch(`/api/game/delete/${gameId}`, { method: 'DELETE', credentials: 'include' })
...
export async function fetchGameTrash(): Promise<TrashedGame[]> {
  ...
  if (!response.ok) throw new Error(`Failed to fetch game trash: ${response.status} ${response.statusText}`)
  return await response.json()
}

export async function restoreGame(gameId: string): Promise<void> {
  const response = await fetch(`/api/game/${gameId}/restore`, { method: 'POST', credentials: 'include' })
  if (!response.ok) {
    const message = await response.text()
    throw new Error(`Failed to restore game: ${response.status} ${response.statusText} ${message}`)
  }
}