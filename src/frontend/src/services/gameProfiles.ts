...
export async function syncProfileWiseOldMan(
  gameId: string,
  profileId: string,
  username?: string,
): Promise<GameProfile> {
  const response = await fetch(
    `/api/game/${gameId}/profiles/${profileId}/sync-wiseoldman`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to sync WiseOldMan: ${response.status} ${response.statusText} ${message}`,
    );
  }
  return await response.json();
}
...
export async function fetchProfileStatHistory(
  gameId: string,
  profileId: string,
): Promise<StatSnapshot[]> {
  if (import.meta.env.VITE_USE_MOCK_DATA === "true") return [];
  const response = await fetch(
    `/api/game/${gameId}/profiles/${profileId}/stat-history`,
    { credentials: "include" },
  );
  if (!response.ok)
    throw new Error(
      `Failed to fetch stat history: ${response.status} ${response.statusText}`,
    );
  const body = await response.json();
...
export async function fetchGameProfileTrash(
  gameId: string,
): Promise<TrashedGameProfile[]> {
  if (import.meta.env.VITE_USE_MOCK_DATA === "true") return [];
  const response = await fetch(`/api/game/${gameId}/profiles/trash`, {
    credentials: "include",
  });
  if (!response.ok)
    throw new Error(
      `Failed to fetch profile trash: ${response.status} ${response.statusText}`,
    );
  const body = await response.json();
  return body.profiles;
}

export async function restoreGameProfile(
  gameId: string,
  profileId: string,
): Promise<GameProfile> {
  const response = await fetch(
    `/api/game/${gameId}/profiles/${profileId}/restore`,
    {
      method: "POST",
      credentials: "include",
    },
  ...
  if (!response.ok)
    throw new Error(
      `Failed to restore profile: ${response.status} ${response.statusText}`,
    );
  return await response.json();
}
...
export async function listChecklist(
  ...
  const response = await fetch(
    `/api/game/${gameId}/checklist?${params.toString()}`,
    { credentials: "include" },
  );
  if (!response.ok)
    throw new Error(
      `Failed to list checklist: ${response.status} ${response.statusText}`,
    );
  const body = await response.json();
  return body.items;
}
...
export async function createChecklistItem(
  ...
  if (!response.ok)
    throw new Error(
      `Failed to create checklist item: ${response.status} ${response.statusText}`,
    );
  return await response.json();
}
...
export async function reorderChecklist(
  ...
  if (!response.ok)
    throw new Error(
      `Failed to reorder checklist: ${response.status} ${response.statusText}`,
    );
}
...
export async function updateChecklistItem(
  ...
  if (!response.ok)
    throw new Error(
      `Failed to update checklist item: ${response.status} ${response.statusText}`,
    );
  return await response.json();
}
...
export async function deleteChecklistItem(
  ...
  if (!response.ok)
    throw new Error(
      `Failed to delete checklist item: ${response.status} ${response.statusText}`,
    );
}
