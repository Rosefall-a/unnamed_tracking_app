...
export async function fetchWorldMaps(gameId: string): Promise<WorldMapEntry[]> {
  ...
  if (!response.ok)
    throw new Error(
      `Failed to fetch worlds: ${response.status} ${response.statusText}`,
    );
  return await response.json();
}
...
export async function renderWorldMap(
  ...
  if (import.meta.env.VITE_USE_MOCK_DATA === "true") return;
  const response = await fetch(
    `/api/game/${gameId}/world-map/${archiveId}/render`,
    {
      method: "POST",
      credentials: "include",
    },
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Failed to start map render: ${response.status} ${response.statusText} ${message}`,
    );
  }
}
...
export function worldMapThumbnailUrl(
  gameId: string,
  archiveId: string,
): string {
