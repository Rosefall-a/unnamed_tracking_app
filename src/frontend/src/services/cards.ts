...
export async function listCards(filters?: { gameId?: string; setId?: string }): Promise<Card[]> {
  const params = new URLSearchParams()
  if (filters?.gameId) params.set('game_id', filters.gameId)
  if (filters?.setId) params.set('set_id', filters.setId)
  const query = params.toString() ? `?${params.toString()}` : ''
  const response = await fetch(`/api/cards${query}`, { credentials: 'include' })
...
export async function updateCard(id: string, fields: CardFieldUpdate): Promise<Card> {
  const body: Record<string, unknown> = {}
  if ('setId' in fields) body.set_id = fields.setId
  if ('rarity' in fields) body.rarity = fields.rarity
  if ('cardCustomization' in fields) body.card_customization = fields.cardCustomization
...
export async function generatePrestigeChallenge(id: string): Promise<Card> {
  ...
  const raw = await handle<BackendCard>(response, `generate prestige challenge for card ${id}`)
  return mapBackendCard(raw)
}
