...
export async function fetchLibraryExport(): Promise<unknown> {
  ...
  if (!response.ok) {
    throw new Error(`Failed to export library: ${response.status} ${response.statusText}`)
  }
...
export async function fetchBackupStatus(): Promise<BackupStatus> {
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch backup status: ${response.status} ${response.statusText}`)
  }
  return await response.json()
}
...
export async function importLibrary(games: unknown[]): Promise<ImportResult> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to import library: ${response.status} ${response.statusText} ${message}`)
  }
  return await response.json()
}
