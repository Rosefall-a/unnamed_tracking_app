...
export async function listInvitations(): Promise<Invitation[]> {
  ...
  if (!response.ok)
    throw await parseError(response, "Failed to load invitations");
...
export async function createInvitation(
  ...
  if (!response.ok)
    throw await parseError(response, "Failed to send invitation");
  return await response.json();
}
...
export async function resendInvitation(id: string): Promise<Invitation> {
  ...
  if (!response.ok)
    throw await parseError(response, "Failed to resend invitation");
  return await response.json();
}
...
export async function revokeInvitation(id: string): Promise<void> {
  ...
  if (!response.ok)
    throw await parseError(response, "Failed to revoke invitation");
}
