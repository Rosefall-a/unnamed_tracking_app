...
export async function fetchPsnStatus(): Promise<PsnStatus> {
  ...
  if (!response.ok) throw await apiError(response, "Failed to fetch PlayStation status");
...
export async function connectPsn(npssoToken: string): Promise<PsnStatus> {
  ...
  if (!response.ok) throw await apiError(response, "Failed to connect PlayStation account");
...
export async function disconnectPsn(): Promise<void> {
  ...
  if (!response.ok) throw await apiError(response, "Failed to disconnect PlayStation account");
}
