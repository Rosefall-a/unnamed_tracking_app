...
onMounted(async () => {
  try {
    ...
    oidc.issuer_url = result.oidc.issuer_url ?? ""; oidc.client_id = result.oidc.client_id ?? "";
    oidc.scopes = result.oidc.scopes ?? "openid profile email"; oidc.redirect_uri = result.oidc.redirect_uri || defaultRedirectUri();
    oidc.groups_claim = result.oidc.groups_claim ?? "groups"; oidc.admin_group = result.oidc.admin_group ?? "";
  ...
  } catch (err) { error.value = err instanceof Error ? err.message : "Failed to load OIDC settings."; }
...
async function save() {
  ...
  try {
    const payload: Record<string, string> = {
      ...
      oidc_login_button_text: oidc.login_button_text.trim() || "Continue with SSO", oidc_allow_new_users: String(oidc.allow_new_users),
    };
    if (oidc.client_secret) payload.oidc_client_secret = oidc.client_secret;
    const result = await updateDeploymentSettings(payload);
    oidc.redirect_uri = result.oidc.redirect_uri || defaultRedirectUri(); oidc.login_button_text = result.oidc.login_button_text?.trim() || "Continue with SSO";
    oidc.allow_new_users = result.oidc.allow_new_users !== false; oidc.client_secret = ""; saved.value = true;
  } catch (err) { error.value = err instanceof Error ? err.message : "Failed to save OIDC settings."; }
  ...
  <section class="section">
    ...
    <template v-else>
      <div class="grid">
        ...
        <label><span>Client ID</span><input v-model="oidc.client_id" /></label><label><span>Client secret</span><input v-model="oidc.client_secret" type="password" placeholder="Leave blank to keep the saved secret" /></label><label><span>Scopes</span><input v-model="oidc.scopes" /></label>
        <label class="full"><span>Redirect URI</span><input v-model="oidc.redirect_uri" autocomplete="url" /></label><label><span>Groups claim</span><input v-model="oidc.groups_claim" placeholder="groups" /></label><label><span>Admin group</span><input v-model="oidc.admin_group" placeholder="archive-admins" /></label>
      </div>
      <div class="match-panel"><div><strong>Match existing users by</strong><p class="hint">Choose which OIDC identity value is used to connect SSO to an existing local account.</p></div><div class="choices"><label class="choice"><input v-model="oidc.user_match_field" type="radio" value="email" /><span><strong>Email</strong><small>OIDC email → local email</small></span></label><label class="choice"><input v-model="oidc.user_match_field" type="radio" value="username" /><span><strong>Username</strong><small>OIDC preferred_username → local username</small></span></label></div></div>
      <div class="login-panel"><div><strong>Login experience</strong><p class="hint">Choose which method is presented first on the login page. The other method remains available in the login-method dropdown.</p></div><div class="login-controls"><label><span>Default login method</span><select v-model="oidc.default_login_method"><option value="sso">SSO</option><option value="local">Local username & password</option></select></label><label><span>SSO button text</span><input v-model="oidc.login_button_text" maxlength="100" /></label></div></div>
      <div class="policy-panel"><div><strong>Account creation</strong><p class="hint">Allow successfully authenticated OIDC users without an existing local account to be created automatically.</p></div><ToggleButton v-model="oidc.allow_new_users" label="Create new users" /></div>
