    ...
    <template v-else>
      <div class="grid">
        <label>
          ...
          <input v-model="oidc.issuer_url" placeholder="https://login.example.com/realms/archive" />
          <small>Enter the issuer URL, or paste the provider's <code>/.well-known/openid-configuration</code> URL directly.</small>
        </label>
      ...
      <div class="match-panel">
        <div>
          ...
          <p class="hint">Choose which OIDC identity value is used to connect SSO to an existing local account. New OIDC accounts still receive both a username and email.</p>
        </div>
        <div class="choices">
          <label class="choice"><input v-model="oidc.user_match_field" type="radio" value="email" /><span><strong>Email</strong><small>OIDC email → local email</small></span></label>
          <label class="choice"><input v-model="oidc.user_match_field" type="radio" value="username" /><span><strong>Username</strong><small>OIDC preferred_username → local username</small></span></label>
        </div>
      </div>
