...
onMounted(async () => {
  try {
    ...
    error.value =
      e instanceof Error ? e.message : "Failed to load SMTP settings.";
  } finally {
    loading.value = false;
  }
});
  ...
  try {
    const response = await fetch("/api/settings/smtp", {
      ...
      body: JSON.stringify({
        ...
        password_reset_enabled: smtp.password_reset_enabled,
        host: smtp.host.trim(),
        port: smtp.port,
...
async function testSmtp() {
  ...
  try {
    ...
    if (!response.ok)
      throw new Error("SMTP test failed. Check the server logs.");
...
<template>
  <section class="section">
    <h2>SMTP / Email</h2>
    <p class="hint">
      Configure the shared SMTP transport for password resets and future email
      features. SMTP credentials are encrypted on the backend and never returned
      to the browser.
    </p>
    ...
    <template v-else>
      <div class="feature-card">
        ...
        <p class="hint">
          Turn this off to keep SMTP configured without allowing users to
          request password-reset emails. Additional email features can use the
          same transport without changing this flag.
        </p>
      </div>
      ...
      <div class="grid">
        ...
        /></label>
        <label
          ><span>Sender name</span><input v-model="smtp.from_name"
        /></label>
      </div>
      ...
      <p class="hint">
        Use STARTTLS for typical port 587 configurations. Use SSL/TLS for
        providers that expect an implicit TLS connection, commonly on port 465.
      </p>
      <div class="actions">
        ...
        <button
          class="secondary"
          :disabled="testing || saving || !smtp.enabled"
          @click="testSmtp"
        >
