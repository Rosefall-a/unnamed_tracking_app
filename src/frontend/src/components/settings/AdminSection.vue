...
import { deleteUser, listUsers, setUserAdmin, signOutAllUsers } from "../../services/admin";
import {
  createInvitation,
  listInvitations,
  resendInvitation,
...
function requestSignOutEveryone() {
  ...
  <section class="settings-section">
    ...
    <div class="security-panel">
      <div>
        ...
        <p>Immediately invalidate every active login session. This includes your current session.</p>
      </div>
    ...
    <template v-else>
      <div class="section-heading">
        <div><h3>Accounts</h3><p>Existing users keep the same login and management controls.</p></div>
        <button type="button" class="primary-button compact" @click="showInviteForm ? (showInviteForm = false) : openInviteForm()">{{ showInviteForm ? "Cancel" : "+ Invite user" }}</button>
      </div>
        ...
        <table class="user-table">
          ...
          <tbody>
            <tr v-for="invitation in invitations" :key="invitation.id">
              <td>{{ invitation.username }}</td><td>{{ invitation.email }}</td>
              <td><span class="role-badge" :class="{admin:invitation.is_admin}">{{ invitation.is_admin ? "Admin" : "User" }}</span></td>
              <td><span class="status-badge">{{ invitationStatus(invitation) }}</span></td>
              <td class="actions" v-if="!invitation.accepted_at"><button type="button" class="small-button" :disabled="!!invitationActionId" @click="resend(invitation.id)">{{ invitationActionId === invitation.id ? "Working…" : "Resend" }}</button><button type="button" class="small-button danger" :disabled="!!invitationActionId || !!invitation.revoked_at" @click="revoke(invitation.id)">Revoke</button></td>
            </tr>
          </tbody>
        </table>
      </div>
