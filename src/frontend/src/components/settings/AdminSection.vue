...
async function signOutEveryone() {
  showSignOutConfirm.value = false; signingOut.value = true; signOutMessage.value = null;
  try {
  ...
  } catch (err) { error.value = err instanceof Error ? err.message : "Failed to sign out all users"; }
...
async function confirmDeleteUser() {
  ...
  try { await deleteUser(deletingUser.value.id); deletingUser.value = null; await loadUsers(); }
  catch (err) { deleteError.value = err instanceof Error ? err.message : "Failed to delete user"; }
...
async function toggleAdmin(user: AdminUser) {
  ...
  } catch (err) { error.value = err instanceof Error ? err.message : "Failed to update user"; }
...
function openCreateForm() {
  ...
  createIsAdmin.value = false; showCreatePassword.value = false;
  createError.value = null; createMessage.value = null; showCreateForm.value = true;
}
async function handleCreateUser() {
  ...
  try {
    ...
    showCreateForm.value = false; createUsername.value = ""; createEmail.value = ""; createPassword.value = ""; createIsAdmin.value = false; showCreatePassword.value = false;
    createMessage.value = "User created successfully."; await loadUsers();
  } catch (err) { createError.value = err instanceof Error ? err.message : "Failed to create user"; }
...
async function handleInviteUser() {
  ...
  try {
    ...
    inviteMessage.value = "Invitation sent. The user can finish setting up their account from the email."; invitations.value = await listInvitations();
  } catch (err) { inviteError.value = err instanceof Error ? err.message : "Failed to send invitation"; }
...
<template>
  <section class="settings-section">
    ...
    <div class="security-panel"><div><h3>Session management</h3><p>Immediately invalidate every active login session. This includes your current session.</p></div><button type="button" class="danger-button" :disabled="signingOut" @click="requestSignOutEveryone">{{ signingOut ? "Signing out…" : "Sign out all users" }}</button></div>
    <p v-if="signOutMessage" class="form-success">{{ signOutMessage }}</p>
    <p v-if="loading">Loading…</p><p v-else-if="error" class="form-error">{{ error }}</p>
    <template v-else>
      <div class="section-heading"><div><h3>Accounts</h3><p>Existing users can be created directly, or invited to choose their own password.</p></div><div class="heading-actions"><button type="button" class="secondary-button compact" @click="showCreateForm ? (showCreateForm = false) : openCreateForm()">{{ showCreateForm ? "Cancel create" : "+ Create user" }}</button><button v-if="smtpReady" type="button" class="primary-button compact" @click="showInviteForm ? (showInviteForm = false) : openInviteForm()">{{ showInviteForm ? "Cancel invite" : "+ Invite user" }}</button><span v-else class="smtp-required">Configure and enable SMTP to invite users.</span></div></div>
      <form v-if="showCreateForm" class="create-form" @submit.prevent="handleCreateUser"><p class="form-hint">Create the account immediately and set its initial password here.</p><label class="field"><span>Username</span><input v-model="createUsername" type="text" required autocomplete="username" /></label><label class="field"><span>Email</span><input v-model="createEmail" type="email" required autocomplete="email" /></label><label class="field"><span>Password</span><input v-model="createPassword" :type="showCreatePassword ? "text" : "password"" required autocomplete="new-password" /></label><label class="password-toggle"><input v-model="showCreatePassword" type="checkbox" /> Show password</label><ToggleButton v-model="createIsAdmin" label="Grant admin access">Grant admin access</ToggleButton><div v-if="createError" class="form-error">{{ createError }}</div><button type="submit" class="primary-button" :disabled="creating">{{ creating ? "Creating user…" : "Create user" }}</button></form>
      <p v-if="createMessage" class="form-success">{{ createMessage }}</p>
      <form v-if="showInviteForm" class="create-form" @submit.prevent="handleInviteUser"><p class="form-hint">We'll email the invitation link. The invited person chooses their password when they accept it.</p><label class="field"><span>Username</span><input v-model="inviteUsername" type="text" required autocomplete="username" /></label><label class="field"><span>Email</span><input v-model="inviteEmail" type="email" required autocomplete="email" /></label><ToggleButton v-model="inviteIsAdmin" label="Grant admin access">Grant admin access</ToggleButton><div v-if="inviteError" class="form-error">{{ inviteError }}</div><button type="submit" class="primary-button" :disabled="inviting">{{ inviting ? "Sending invitation…" : "Send invitation" }}</button></form>
      <p v-if="inviteMessage" class="form-success">{{ inviteMessage }}</p>
      <table class="user-table"><thead><tr><th>Username</th><th>Email</th><th>Role</th><th>Joined</th><th></th></tr></thead><tbody><tr v-for="user in users" :key="user.id"><td>{{ user.username }}</td><td>{{ user.email }}</td><td><span class="role-badge" :class="{admin:user.is_admin}">{{ user.is_admin ? "Admin" : "User" }}</span></td><td class="joined">{{ user.created_at ? new Date(user.created_at * 1000).toLocaleDateString() : "N/A" }}</td><td class="actions"><button type="button" class="small-button" :disabled="user.id === currentUser?.id || togglingAdminId === user.id" @click="toggleAdmin(user)">{{ user.is_admin ? "Demote" : "Promote" }}</button><button type="button" class="small-button danger" :disabled="user.id === currentUser?.id" @click="deletingUser = user; deleteError = null">Delete</button></td></tr></tbody></table>
      <div v-if="invitations.length" class="invitation-list"><div class="section-heading"><div><h3>Invitations</h3><p>Pending invitations expire after 7 days and can be resent or revoked.</p></div></div><table class="user-table"><thead><tr><th>Username</th><th>Email</th><th>Role</th><th>Status</th><th></th></tr></thead><tbody><tr v-for="invitation in invitations" :key="invitation.id"><td>{{ invitation.username }}</td><td>{{ invitation.email }}</td><td><span class="role-badge" :class="{admin:invitation.is_admin}">{{ invitation.is_admin ? "Admin" : "User" }}</span></td><td><span class="status-badge">{{ invitationStatus(invitation) }}</span></td><td class="actions" v-if="!invitation.accepted_at"><button type="button" class="small-button" :disabled="!!invitationActionId" @click="resend(invitation.id)">{{ invitationActionId === invitation.id ? "Working…" : "Resend" }}</button><button type="button" class="small-button danger" :disabled="!!invitationActionId || !!invitation.revoked_at" @click="revoke(invitation.id)">Revoke</button></td></tr></tbody></table></div>
    </template>
