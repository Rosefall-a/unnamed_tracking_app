...
export function startTask(label: string, total: number, options: { indeterminate?: boolean } = {}): string {
  const id = crypto.randomUUID()
  tasks.push({ id, label, done: 0, total, status: 'running', feed: [], indeterminate: options.indeterminate })
  return id
}

export function updateTask(id: string, done: number, total?: number, speedLabel?: string) {
