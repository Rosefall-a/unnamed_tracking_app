...
from src.features.imports.yamtrack import MAX_BYTES, YamtrackEntry, YamtrackImportError, parse_yamtrack
...
async def _read(file: UploadFile) -> list[YamtrackEntry]:
...
async def import_yamtrack(
    ...
    matches = await match_titles(db, current_user.id, _match_items(entries))
    created = updated = kept = episodes_imported = 0
    ...
    for entry, match in zip(entries, matches):
        ...
        if match.existing is not None:
            ...
            if picked:
                ...
                if isinstance(match.existing, TVShow):
                    episodes_imported += _apply_progress(match.existing, entry)
                updated += 1
            else:
                kept += 1
    ...
    if fetch_details and touched:
        ...
        # fill_details may create seasons for a completed show; restore the
        # exact partial progress from YamTrack after metadata is available.
        for entry, (item, row, _) in zip(entries, touched):
            if isinstance(row, TVShow):
                episodes_imported += _apply_progress(row, entry)
