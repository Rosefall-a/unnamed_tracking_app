    ...
    try:
        ...
        if payload.oidc_enabled:
            ...
            if oidc is None:
                oidc = OidcSettings()
                db.add(oidc)
            oidc.issuer_url = oidc_values["issuer_url"]
            oidc.client_id = oidc_values["client_id"]
            oidc.client_secret = encrypt_secret(client_secret)
            oidc.scopes = oidc_values["scopes"]
            oidc.redirect_uri = oidc_values["redirect_uri"]
            oidc.groups_claim = oidc_values["groups_claim"]
            oidc.admin_group = oidc_values["admin_group"]
            oidc.user_match_field = oidc_values["user_match_field"]
