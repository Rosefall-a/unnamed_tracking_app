...
async def password_reset_status(db: AsyncSession = Depends(get_db)) -> dict[str, bool]:
    ...
    enabled = bool(settings_row and settings_row.smtp_enabled and settings_row.password_reset_enabled and settings_row.smtp_host and settings_row.smtp_from_email)
...
async def request_password_reset(payload: PasswordResetRequest, request: Request, db: AsyncSession = Depends(get_db)) -> dict[str, str]:
    ...
    settings_row = await db.scalar(select(AppIntegrationSettings).limit(1))
    if settings_row is None or not settings_row.smtp_enabled or not settings_row.password_reset_enabled:
...
@router.post("/confirm")
async def confirm_password_reset(payload: PasswordResetConfirm, response: Response, db: AsyncSession = Depends(get_db)) -> dict[str, str]:
    ...
    return {"message": "Your password has been reset. You are now signed in.", "user_id": str(user.id)}
