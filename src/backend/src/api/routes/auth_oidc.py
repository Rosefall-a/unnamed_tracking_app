...
async def _get_config(db,slug="default"):
    ...
    if row and slug!="default":
        for provider in _named_rows(row):
            if provider.get("slug")==slug and provider.get("client_secret"):return _config_from_provider(provider)
        return None
...
async def oidc_status(db:AsyncSession=Depends(get_db)):
    ...
    if row:
        for provider in _named_rows(row):
            if provider.get("show_on_login",True):providers.append({"name":provider.get("name",provider["slug"]),"slug":provider["slug"],"button_text":provider.get("button_text") or "Continue with SSO","button_image_url":provider.get("button_image_url")})
...
def _safe_username(value,email):
    username="".join(c for c in value.strip() if c.isalnum() or c in "._-")[:100];return username or email.split("@",1)[0][:90] or f"user-{secrets.token_hex(4)}"
async def _complete_callback(request,db,config,client_name):
    ...
    session_token=secrets.token_urlsafe(32);db.add(UserSession(user_id=user.id,token_hash=hash_token(session_token),expires_at=int(time.time())+_SESSION_SECONDS));await db.commit();response=RedirectResponse("/login?oidc=success",303);response.set_cookie(key=SESSION_COOKIE,value=session_token,max_age=_SESSION_SECONDS,httponly=True,samesite="lax",secure=settings.AUTH_COOKIE_SECURE,path="/");return response
