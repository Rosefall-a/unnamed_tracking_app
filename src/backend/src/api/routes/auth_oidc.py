...
def _env_config():
    ...
    return OidcConfig(issuer_url=issuer, client_id=settings.OIDC_CLIENT_ID, client_secret=settings.OIDC_CLIENT_SECRET, scopes=settings.OIDC_SCOPES, redirect_uri=settings.OIDC_REDIRECT_URI, groups_claim=settings.OIDC_GROUPS_CLAIM, admin_group=settings.OIDC_ADMIN_GROUP, user_match_field=getattr(settings, "OIDC_USER_MATCH_FIELD", "email"), discovery_url=issuer if issuer.endswith("/.well-known/openid-configuration") else None)
...
def _named_rows(row):
    try:
        ...
        return []
    return [provider for provider in data if isinstance(provider, dict) and provider.get("slug") and provider.get("enabled", True)]


def _config_from_provider(provider):
    issuer = str(provider["issuer_url"]).strip()
    return OidcConfig(issuer_url=issuer, client_id=str(provider["client_id"]), client_secret=decrypt_secret(str(provider["client_secret"])), scopes=provider.get("scopes") or "openid profile email", redirect_uri=provider.get("redirect_uri") or None, groups_claim=provider.get("groups_claim") or "groups", admin_group=provider.get("admin_group") or None, user_match_field=provider.get("user_match_field") or "email", allow_new_users=bool(provider.get("allow_new_users", True)), discovery_url=issuer if issuer.endswith("/.well-known/openid-configuration") else None, name=provider.get("name") or provider["slug"], slug=provider["slug"], button_text=provider.get("button_text") or "Continue with SSO", button_image_url=provider.get("button_image_url"))
    ...
    if row:
        for provider in _named_rows(row):
            if provider.get("show_on_login", True):
                providers.append({"name": provider.get("name", provider["slug"]), "slug": provider["slug"], "button_text": provider.get("button_text") or "Continue with SSO", "button_image_url": provider.get("button_image_url"), "button_color": provider.get("button_color") or "#d68a34"})
    if not providers and config:
        providers = [{"name": config.name, "slug": "default", "button_text": row.login_button_text.strip() if row and row.login_button_text.strip() else config.button_text, "button_image_url": config.button_image_url, "button_color": "#d68a34"}]
...
async def oidc_provider_login(provider_slug: str, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if config is None: return RedirectResponse("/login", 303)
    return await begin_oidc(request, config)
...
def _match_value(claims, field, email):
    if field == "username": return str(claims.get("preferred_username") or claims.get("name") or "").strip()
    return email
...
async def _complete_callback(request, db, config, client_name):
    ...
    try:
        ...
        claims = dict(token.get("userinfo") or await client.userinfo(token=token)); claims.update({k: v for k, v in token.items() if k not in claims})
    except Exception:
    ...
    if user is None:
        ...
        if user.oidc_subject and user.oidc_subject not in {linked_subject, subject}: return RedirectResponse("/login?oidc_error=identity_conflict", 303)
        user.oidc_subject = linked_subject; user.email = email
        if config.admin_group: user.is_admin = is_admin
    session_token = secrets.token_urlsafe(32); db.add(UserSession(user_id=user.id, token_hash=hash_token(session_token), expires_at=int(time.time()) + _SESSION_SECONDS)); await db.commit()
    response = RedirectResponse("/login?oidc=success", 303); response.set_cookie(key=SESSION_COOKIE, value=session_token, max_age=_SESSION_SECONDS, httponly=True, samesite="lax", secure=settings.AUTH_COOKIE_SECURE, path="/"); return response


@router.get("/callback", name="oidc_callback")
async def oidc_callback(request: Request, db: AsyncSession = Depends(get_db)):
    config = await _get_config(db)
    if config is None: return RedirectResponse("/login?oidc_error=not_configured", 303)
