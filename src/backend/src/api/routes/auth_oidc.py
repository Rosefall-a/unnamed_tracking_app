...
async def _complete_callback(request,db,config,client_name):
    ...
    if user is None:
        ...
        if user.oidc_subject and user.oidc_subject!=linked_subject and user.oidc_subject!=subject:return RedirectResponse("/login?oidc_error=identity_conflict",303)
