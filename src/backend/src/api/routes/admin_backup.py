...
def _key_copies() -> list[str]:
    ...
    for name in ("fernet.key", "fernet.key.1", "fernet.key.2"):
        try:
            value = (DATA_ROOT / "config" / name).read_text(encoding="utf-8").strip()
...
async def inspect_secret_backup(
    ...
    if not isinstance(app, dict) or not isinstance(oidc, dict):
        raise HTTPException(400, "Backup is missing deployment settings.")
    return {
        "format_version": backup.get("format_version", 1),
        "exported_at": backup.get("exported_at"),
        "sections": [
            {
                "id": "application",
                "label": "Application and integrations",
                "description": "Provider credentials, SMTP settings and application runtime settings.",
                "available": True,
            },
            {
                "id": "oidc",
                "label": "OIDC / SSO",
                "description": "OIDC configuration and named provider credentials.",
                "available": True,
            },
            {
                "id": "encryption",
                "label": "Encryption keys",
                "description": "Persistent Fernet key copies used to encrypt stored deployment secrets. Selecting this also restores the encrypted settings that belong to those keys.",
                "available": True,
            },
        ],
    }
...
async def import_secret_backup(
    ...
    del admin
    try:
        selected = json.loads(sections)
    except (TypeError, ValueError) as exc:
        raise HTTPException(400, "Invalid restore selection.") from exc
    if (
        not isinstance(selected, list)
        or not selected
        or any(s not in {"application", "oidc", "encryption"} for s in selected)
    ):
    ...
    if "encryption" in selected:
        selected.update({"application", "oidc"})
    ...
    return {
        ...
        "message": "Deployment configuration restored. Choose Sign out now or refresh the page if this session was invalidated."
        if sessions_revoked
        else "Selected deployment configuration restored without invalidating active sessions.",
    }
