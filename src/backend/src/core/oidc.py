...
class OidcConfig:
    ...
    def server_metadata_url(self) -> str:
        ...
        if configured.endswith(suffix): return configured
...
def register_oidc_provider(config: OidcConfig, client_name: str = "oidc") -> None:
    oauth.register(name=client_name,client_id=config.client_id,client_secret=config.client_secret,server_metadata_url=config.server_metadata_url,client_kwargs={"scope":config.scopes},overwrite=True)

def callback_url(request: Request, config: OidcConfig) -> str:
    if config.redirect_uri: return config.redirect_uri
    return str(request.url_for("oidc_callback_provider", provider_slug=config.slug)) if config.slug != "default" else str(request.url_for("oidc_callback"))
...
async def begin_oidc(request: Request, config: OidcConfig) -> RedirectResponse:
    ...
    if client is None: raise HTTPException(status_code=503, detail="OIDC provider is unavailable.")
