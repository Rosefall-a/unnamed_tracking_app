...
async def cleanup_expired_authentication_records(
    ...
    handoff_result = await db.execute(
        delete(MobileOidcHandoff).where(MobileOidcHandoff.expires_at <= current_time)
    )
    await db.commit()
    ...
    counts = (
        ...
        handoff_result.rowcount or 0,
    )
    logger.info(
        "Authentication cleanup removed %d invitations, %d password reset tokens, %d sessions, and %d mobile OIDC handoffs",
        *counts,
    )
    return counts
