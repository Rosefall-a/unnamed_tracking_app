...
from src.api.routes import (
    app_integrations, api_keys, auth, bounties, cards, default_game_assets,
    export_import, game_archives, games, library_sync, media, settings, stats, users,
)
...
async def bootstrap_primary_user() -> None:
    async with SessionLocal() as db:
        if app_settings.PRIMARY_USER_USERNAME.strip() and app_settings.PRIMARY_USER_EMAIL.strip() and app_settings.PRIMARY_USER_PASSWORD:
